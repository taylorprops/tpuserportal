            ...
            <li class="py-1 border-b">
                ...
                @if (auth() -> user() -> id == $note -> user_id)
                    <div class="flex justify-end">
                        <div>
                            @if ($note -> read == 0)
                                <button type="button" class="bg-red-100 text-red-500 hover:text-red-600 p-1 pl-2 flex items-center rounded-full" @click="delete_note($el, {{ $note -> event_id }}, {{ $note -> id }})">Delete <i class="fa-duotone fa-times-circle  fa-lg ml-2"></i></button>
                            @else
