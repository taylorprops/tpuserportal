                ...
                @else
                    <div class="flex justify-end">
                        <div x-data="{ show_mark_read: '{{ $note -> read == 0 ? 'yes' : 'no' }}' }">
                            <button type="button" class="button success sm" @click="mark_note_read($el, {{ $note -> event_id }}, {{ $note -> id }}); show_mark_read = 'no'"
                                x-show="show_mark_read === 'yes'"><i class="fal fa-check mr-2"></i> Mark Read</button>
