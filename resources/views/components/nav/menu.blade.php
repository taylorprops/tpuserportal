...
@if ($level === '1')
    <li>
        <a href="{{ $link }}"
            class="menu-link flex items-center h-full w-full px-2 py-1 text-sm font-medium rounded-md group text-primary-dark hover:text-white hover:bg-primary focus:bg-primary focus:text-white">
            <div
                class="flex justify-center items-center h-6 w-6 mr-4 rounded bg-white group-hover:bg-primary-light group-hover:text-white group-focus:bg-primary-light group-focus:text-white">
                <i class="{{ $icon }}"></i>
            </div>
            <span class="group-hover:text-white group-focus:text-white">{{ $title }}</span>
        </a>
    </li>
@endif
...
@if ($level === '2')
    ...
    <li>
        ...
        <div class="px-2 py-1 text-sm font-medium cursor-pointer rounded-md group text-primary-dark hover:text-white hover:bg-primary focus:bg-primary"
            ...
            <ul class="bg-white rounded-md p-1 mt-2" x-show="active_menu === {{ $id }}" x-transition:enter="transition ease-in-out duration-300"
                x-transition:enter-start="opacity-0 transform scale-y-0 -translate-y-1/2" x-transition:enter-end="opacity-100 transform scale-y-100 translate-y-0"
                x-transition:leave="transition ease-in-out duration-300" x-transition:leave-start="opacity-100 transform scale-y-100 translate-y-0"
                ...
                @foreach ($level3 as $link)
                    ...
                    @else
                        <li class="text-sm text-primary-dark rounded-md hover:text-primary hover:bg-gray-100"
                            ...
                            <ul class="bg-gray-100 p-2 pt-0 ml-2 rounded-md" x-show="active_sub_menu === {{ $id }}"
                                x-transition:enter="transition ease-in-out duration-300" x-transition:enter-start="opacity-0 transform scale-y-0 -translate-y-1/2"
                                x-transition:enter-end="opacity-100 transform scale-y-100 translate-y-0" x-transition:leave="transition ease-in-out duration-300"
                                x-transition:leave-start="opacity-100 transform scale-y-100 translate-y-0" x-transition:leave-end="opacity-0 transform scale-y-0 -translate-y-1/2">
