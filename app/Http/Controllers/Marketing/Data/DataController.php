...
class DataController extends Controller
{
    ...
    public function get_results(Request $request)
    {
        ...
        if ($list_group == 'test_center') {
            ...
            $agents = TestCenterAddresses::select($agent_columns) -> whereIn('state', $states) -> get();
    ...
    public function add_new_list(Request $request)
    {
        ...
        $file_name = 'AgentRoster.csv';
        $file -> storeAs('tmp', $file_name);

        $file_location = Storage::path('/tmp/'.$file_name);
        ...
        if ($type == 'in_house') {
            ...
            //Excel::load($file_location) -> convert('xls');

            // send to Kristen
            $tos = ['k.hayghe@heritagetitle.com', 'miketaylor0101@gmail.com'];

            $body = 'Attached is an updated Taylor Properties agent roster.
            <br><br>
            Thanks,<br>
            Mike';
